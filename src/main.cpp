...
#if defined(NDEBUG)
# error "Gnuccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xecbb8f468e9b9af79a8e0bd0b4bd709004857a6d06d536ac7f5167df32a46b44");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gnuccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gnuccoin Signed Message:\n";
...
{
    // Gnuccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Gnuccoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Gnuccoin: 3.5 days
static const int64 nTargetSpacing = 10 * 60; // Gnuccoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gnuccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gnuccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x43e8303a17f1895f32c328636d753a88f591aea14bd6256b75b88f19eb0ec2bf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Gnucco Klasias Cerasale 1988";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0434e83834f1c492e2c33f7c84fda3fc618977b2865303b23e49719ce5ecaf483c7fc770b531f28bc057c53fbe4012eab9a46b20dde1783f2f53324e5537089b63") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1544283326;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086226771;

        if (fTestNet)
        {
            block.nTime    = 1544283277;
            block.nNonce   = 385932276;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x82c93bb32d53a5c645ffb156786926329068866f69146e809747485a963bf23a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xbb, 0xd4 }; // Gnuccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GnuccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GnuccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GnuccoinMiner(CWallet *pwallet)
{
    printf("GnuccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gnuccoin-miner");
    ...
    try { loop {
        ...
        printf("Running GnuccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GnuccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GnuccoinMiner, pwallet));
}
